...
object listener {
            ...
            case SubGoalMessage(_,_,_,r) =>
              message.goal match {
                ...
                case listener.listen =>
                  listener.listen.execute(message.params.asInstanceOf[listener.listen.Parameters])
  ...
  object Agent extends IAgent {
    ...
    override def agent_type: String = "listener"
    ...
    def create_goal_message(t: StructTerm,r:String,ref:ActorRef[IMessage]) : Option[SubGoalMessage] = {
      if(t.functor=="listen" && t.terms.size == 1 ) {
        val args : listen.Parameters = if(t.terms.size == 0)  listen.Parameters(List( )) else listen.Parameters(t.terms.toList)
        Option(SubGoalMessage(listen, args,r,ref))
      } else  {
  ...
  object listen extends IGoal {
    ...
    def execute(params: Parameters) (implicit executionContext: ExecutionContext) : Unit = {
      ...
      val m0 = executionContext.beliefBase.matchTerms(/* StructTerm("listen",Seq[GenericTerm](VarManager.bindVar("M", vars))) All vars no need to check */);
    ...
    def plan0(vars: HashMap[String,GenericTerm])(implicit executionContext: ExecutionContext): Unit = {
      ...
      if(( vars("M") == IntTerm(9999) ).holds) {
        PrimitiveAction.execute(PrimitiveAction.Parameters(() => println( StringTerm("done") + executionContext.agentLogger.timeTaken )))
      }
