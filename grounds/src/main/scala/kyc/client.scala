...
object client {
      ...
      Behaviors.setup { context =>
        ...
        Behaviors.receive {
          (context, message) => message match {
            ...
            case IntentionFailedMessage(_,_) =>
              context.log.debug(f"${executionContext.name}: an intention was failed")
  ...
  object be_informed_of_acceptance extends IGoal {
    ...
    def plan0(vars: HashMap[String,Term])(implicit executionContext: ExecutionContext): Unit = {
      ...
      BeliefUpdateAction.execute(BeliefUpdateAction.Parameters("-", Struct.of("applying_to",VarManager.bindVar("Bank", vars))))
      PrimitiveAction.execute(PrimitiveAction.Parameters(() => println( "Yaay! I am now a client of " + vars("Bank") )))
  ...
  object change_data extends IGoal {
    ...
    def plan0(vars: HashMap[String,Term])(implicit executionContext: ExecutionContext): Unit = {
      ...
      BeliefUpdateAction.execute(BeliefUpdateAction.Parameters("-", Struct.of("my_sib",VarManager.bindVar("_", vars))))
  ...
  object need_to_update_data extends IGoal {
    ...
    def plan0(vars: HashMap[String,Term])(implicit executionContext: ExecutionContext): Unit = {
      ...
      val ex_L97155 = executionContext.beliefBase.rawQuery( Struct.of("client_of",VarManager.bindVar("L97155", vars)) )
      while (ex_L97155.hasNext) {
        val sol_L97155 = ex_L97155.next
        if(sol_L97155.isYes) {
          vars += ("B" -> sol_L97155.getSubstitution.get( VarManager.bindVar("L97155", vars)).asInstanceOf[Term])
